...
How to build and run the vCenter Inventory app on Windows 10/11, from a clean
...
```powershell
git clone https://github.com/dalehassinger/VMware-Explore-Hackathon-2026-Live.git
```

```powershell
cd VMware-Explore-Hackathon-2026-Live
```
...
```
%APPDATA%\com.vcrocs.vcenter-inventory\config.json
```

which expands to `C:\Users\<you>\AppData\Roaming\com.vcrocs.vcenter-inventory\`.

Open the folder with:

```powershell
explorer "$env:APPDATA\com.vcrocs.vcenter-inventory"
```
...
currently *vCenter Inventory* 0.1.0, so expect something close to
`vCenter Inventory_0.1.0_x64_en-US.msi` and
`vCenter Inventory_0.1.0_x64-setup.exe`.
...
| Config file | `explorer "$env:APPDATA\com.vcrocs.vcenter-inventory"` | anywhere |
